(display
    (let [(hands '(
        ...
        (let* [(has-flush? (lambda (cards)
                (apply char=? (map (lambda (x) (string-ref x 1)) cards))))
               (card-values (lambda (cards)
                (sort < (map (lambda (x)
                    (let [(x1 (string-ref x 0))]
                        (if (and (char>=? x1 #\0) (char<=? x1 #\9))
                            (- (char->integer x1) (char->integer #\0))
                            (cond
                                ((char=? x1 #\T) 10)
                                ((char=? x1 #\J) 11)
                                ((char=? x1 #\Q) 12)
                                ...
                                ((char=? x1 #\A) 14)
                                (else #f))))) cards))))
              (has-straight? (lambda (card-values)
                (let helper [(first (car card-values)) (next (cdr card-values))]
                    (cond
                        ((not (pair? next)) #t)
                        ((= (+ first 1) (car next)) (helper (car next) (cdr next)))
                        (else #f)))))
              (group-card-values (lambda (card-values)
                (let helper [(cards card-values) (result '()) (current-match-list '())]
                    (cond
                        ((not (pair? cards)) (if (pair? current-match-list) (reverse (cons current-match-list result)) (reverse result)))
                        ((not (pair? current-match-list)) (helper (cdr cards) result (cons (car cards) current-match-list)))
                        ((eq? (car current-match-list) (car cards)) (helper (cdr cards) result (cons (car cards) current-match-list)))
                        (else (helper (cdr cards) (cons current-match-list result) (list (car cards))))))))
              (hand-score (lambda (cards card-values) ; returns rank, then tie breaker card(s)
                (let [(has-flush (has-flush? cards)) (has-straight (has-straight? card-values)) (grouped-card-values (group-card-values card-values))]
                    (cond
                        ((and has-flush has-straight (= (list-ref card-values 4) 13)) (list 10 13)) ; Royal flush
                        ((and has-flush has-straight) (list 9 (list-ref card-values 4))) ; Straight flush
                        ((= 2 (length grouped-card-values))
                            (cond
                                ((= 1 (length (car grouped-card-values))) (list 8 (caadr grouped-card-values) (caar grouped-card-values))) ; Four-of-a-kind
                                ((= 2 (length (car grouped-card-values))) (list 7 (caadr grouped-card-values) (caar grouped-card-values))) ; Full house
                                ((= 3 (length (car grouped-card-values))) (list 7 (caar grouped-card-values) (caadr grouped-card-values))) ; Full house
                                ((= 4 (length (car grouped-card-values))) (list 8 (caar grouped-card-values) (caadr grouped-card-values))) ; Four-of-a-kind
                        ))
                        (has-flush (list 6 (list-ref card-values 4))) ; Flush
                        (has-straight (list 5 (list-ref card-values 4))) ; Straight
                        ((= 3 (length grouped-card-values))
                            (cond
                                ((= 3 (length (car grouped-card-values))) (list 4 (caar grouped-card-values) (caaddr grouped-card-values) (caadr grouped-card-values))) ; Three-of-a-kind (low card)
                                ((= 3 (length (cadr grouped-card-values))) (list 4 (caadr grouped-card-values) (caaddr grouped-card-values) (caar grouped-card-values))) ; Three-of-a-kind (middle card)
                                ((= 3 (length (caddr grouped-card-values))) (list 4 (caaddr grouped-card-values) (caadr grouped-card-values) (caar grouped-card-values))) ; Three-of-a-kind (high card)
                                ((and (= 2 (length (car grouped-card-values))) (= 2 (length (cadr grouped-card-values)))) (list 3 (caadr grouped-card-values) (caar grouped-card-values) (caaddr grouped-card-values))) ; Two pairs (low+middle)
                                ((and (= 2 (length (car grouped-card-values))) (= 2 (length (caddr grouped-card-values)))) (list 3 (caaddr grouped-card-values) (caar grouped-card-values) (caadr grouped-card-values))) ; Two pairs (low+high)
                                ((and (= 2 (length (cadr grouped-card-values))) (= 2 (length (caddr grouped-card-values)))) (list 3 (caaddr grouped-card-values) (caadr grouped-card-values) (caar grouped-card-values))) ; Two pairs (middle+high)
                        ))
                        ((= 4 (length grouped-card-values))
                            (cond
                                ((= 2 (length (car grouped-card-values))) (list 2 (caar grouped-card-values) (caaddr (cdr grouped-card-values)) (caaddr grouped-card-values) (caadr grouped-card-values))) ; Pair (low)
                                ((= 2 (length (cadr grouped-card-values))) (list 2 (caadr grouped-card-values) (caaddr (cdr grouped-card-values)) (caaddr grouped-card-values) (caar grouped-card-values))) ; Pair (second-lowest)
                                ((= 2 (length (caddr grouped-card-values))) (list 2 (caaddr grouped-card-values) (caaddr (cdr grouped-card-values)) (caadr grouped-card-values) (caar grouped-card-values))) ; Pair (second-highest)
                                ((= 2 (length (cadddr grouped-card-values))) (list 2 (caaddr (cdr grouped-card-values)) (caaddr grouped-card-values) (caadr grouped-card-values) (caar grouped-card-values))) ; Pair (highest)
                        ))
                        (else (list 1 (list-ref card-values 4)))))))]
            (let hand-loop [(hands hands) (p1-wins 0)]
                (if (not (pair? hands))
                    p1-wins
                    (let [(p1-cards (caar hands)) (p2-cards (cdar hands))]
                        (let [(p1-values (card-values p1-cards)) (p2-values (card-values p2-cards))]
                            (let [(p1-score (hand-score p1-cards p1-values)) (p2-score (hand-score p2-cards p2-values))]
                                (let score-loop [(p1 p1-score) (p2 p2-score)]
                                    (cond
                                        ((not (pair? p1)) #f)
                                        ((< (car p1) (car p2)) (hand-loop (cdr hands) p1-wins))
                                        ((> (car p1) (car p2)) (hand-loop (cdr hands) (+ p1-wins 1)))
                                        (else (score-loop (cdr p1) (cdr p2)))))))))))))
(display #\newline)
(exit)
